...
export default function CotisationsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 overflow-x-hidden">
      ...
      <div className="bg-white border-b border-gray-200 px-3 py-3 sm:px-6 lg:px-8">
        <div className="flex flex-col space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
            <div className="w-full">
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Cotisations</h1>
              <p className="text-sm sm:text-base text-gray-600 mt-1">Gérez les cotisations de vos membres</p>
            </div>
            <button className="w-full sm:w-auto bg-purple-600 text-white px-4 py-3 rounded-lg hover:bg-purple-700 transition-colors flex items-center justify-center space-x-2 text-base font-semibold">
          ...
          {/* Montant total */}
          <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg p-4 text-white w-full">
            <div className="flex items-center space-x-2">
              <DollarSign className="w-5 h-5" />
              <span className="text-sm font-medium">Montant total perçu</span>
            </div>
            <div className="text-2xl sm:text-3xl font-bold mt-1">{stats.totalAmount.toLocaleString()} FCFA</div>
          </div>
        </div>
      </div>

      {/* Recherche et filtres mobiles */}
      <div className="bg-white border-b border-gray-200 px-4 py-4 sm:px-6 lg:px-8 w-full">
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-base sm:text-base transition-shadow focus:shadow-lg"
            />
          </div>
          {/* Filtres rapides */}
          <div className="flex items-center space-x-2 overflow-x-auto pb-1">
            <button
              ...
              className={`flex-shrink-0 w-full sm:w-auto px-3 py-2 rounded-lg text-base font-medium transition-colors ${statusFilter === 'ALL' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
            >Toutes</button>
            <button
              onClick={() => setStatusFilter(PaymentStatus.PAID)}
              className={`flex-shrink-0 w-full sm:w-auto px-3 py-2 rounded-lg text-base font-medium transition-colors ${statusFilter === PaymentStatus.PAID ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
            >Payées</button>
            <button
              onClick={() => setStatusFilter(PaymentStatus.PENDING)}
              className={`flex-shrink-0 w-full sm:w-auto px-3 py-2 rounded-lg text-base font-medium transition-colors ${statusFilter === PaymentStatus.PENDING ? 'bg-yellow-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
            >En attente</button>
            <button
              onClick={() => setStatusFilter(PaymentStatus.OVERDUE)}
              className={`flex-shrink-0 w-full sm:w-auto px-3 py-2 rounded-lg text-base font-medium transition-colors ${statusFilter === PaymentStatus.OVERDUE ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
            >En retard</button>
            <button
              onClick={() => setShowFilters(!showFilters)}
              className="flex-shrink-0 flex items-center space-x-2 px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
          ...
          {/* Panneau de filtres */}
          {showFilters && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 bg-gray-50 rounded-lg w-full">
              <div>
                <label className="block text-base font-medium text-gray-700 mb-1">Année</label>
                <select
                  value={yearFilter}
                  onChange={(e) => setYearFilter(e.target.value === 'ALL' ? 'ALL' : parseInt(e.target.value))}
                  className="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-base"
                >
              ...
              <div>
                <label className="block text-base font-medium text-gray-700 mb-1">Méthode de paiement</label>
                <select
                  value={methodFilter}
      ...
      <div className="px-2 py-4 sm:px-6 lg:px-8 w-full">
        {filteredCotisations.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredCotisations.map((cotisation) => {
              ...
              return (
                <div key={cotisation.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow w-full">
                  <div className="p-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                      <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${statusBadge.className} focus:outline-none focus:ring-2 focus:ring-purple-500`} tabIndex={0}>
                        {statusBadge.label}
                      </span>
                      <span className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full ${getPaymentMethodColor(cotisation.paymentMethod)} focus:outline-none focus:ring-2 focus:ring-purple-500`} tabIndex={0}>
                    ...
                    <div className="mt-4 flex flex-col sm:flex-row items-center justify-between gap-2 w-full">
                      <div className="flex flex-row flex-wrap items-center gap-2 w-full">
                        <button className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 text-base font-medium text-purple-600 bg-purple-50 rounded-md hover:bg-purple-100 transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500">
                          <Eye className="w-5 h-5 mr-2" />
                          Voir
                        </button>
                        <button className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 text-base font-medium text-green-600 bg-green-50 rounded-md hover:bg-green-100 transition-colors focus:outline-none focus:ring-2 focus:ring-green-500">
                          <Download className="w-5 h-5 mr-2" />
                          Reçu
                        </button>
                      </div>
                      <div className="flex flex-row flex-wrap items-center gap-2 w-full">
                        <button className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 text-base font-medium text-blue-600 bg-blue-50 rounded-md hover:bg-blue-100 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500">
                          <Edit className="w-5 h-5 mr-2" />
                          Modifier
                        </button>
                        <button className="inline-flex items-center justify-center w-full sm:w-auto px-4 py-2 textBase font-medium text-red-600 bg-red-50 rounded-md hover:bg-red-100 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500">
      ...
      {/* Pagination mobile */}
      <div className="bg-white border-t border-gray-200 px-4 py-3 sm:px-6 lg:px-8 w-full">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2 w-full">
          <div className="text-base text-gray-700 w-full text-center sm:text-left">
          ...
          <div className="flex flex-row flex-wrap items-center gap-2 w-full justify-center sm:justify-end">
            <button className="w-1/3 sm:w-auto px-4 py-2 text-base text-gray-500 hover:text-gray-700 disabled:opacity-50 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">
              Précédent
            </button>
            <button className="w-1/3 sm:w-auto px-4 py-2 text-base bg-purple-600 text-white rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">
              1
            </button>
            <button className="w-1/3 sm:w-auto px-4 py-2 text-base text-gray-500 hover:text-gray-700 disabled:opacity-50 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">
              Suivant
