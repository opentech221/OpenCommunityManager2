...
interface EditCotisationModalProps {
  isOpen: boolean;
  onClose: () => void;
  ...
  onSave: (data: Partial<CotisationType>) => void;
  isLoading?: boolean;
}

export const EditCotisationModal: React.FC<EditCotisationModalProps> = ({
  isOpen,
  onClose,
  cotisation,
  members,
  ...
  // Initialiser le formulaire avec les données de la cotisation
  useEffect(() => {
    if (cotisation && isOpen) {
      setFormData({
        memberId: cotisation.memberId,
        amount: cotisation.amount.toString(),
        paymentDate: safeDateToInputString(cotisation.paymentDate),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (validateForm()) {
      const updatedData: Partial<CotisationType> = {
      ...
      <div className="bg-white rounded-xl max-w-md w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Modifier la cotisation</h2>
              <p className="text-sm text-gray-500">ID: #{cotisation.id}</p>
            </div>
          </div>
          <button
            onClick={handleClose}
