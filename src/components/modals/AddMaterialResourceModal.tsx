...
interface AddMaterialResourceModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (resource: Omit<MaterialResourceType, 'id'>) => void;
}

const AddMaterialResourceModal: React.FC<AddMaterialResourceModalProps> = ({ isOpen, onClose, onSave }) => {
  const [formData, setFormData] = useState({
    name: '',
    category: MaterialCategoryEnum.AUTRE as MaterialCategory,
    description: '',
    condition: 'BON' as 'EXCELLENT' | 'BON' | 'MOYEN' | 'MAUVAIS',
    location: '',
    purchaseDate: '',
    purchasePrice: '',
    currentValue: '',
    responsible: '',
    availability: 'DISPONIBLE' as 'DISPONIBLE' | 'UTILISÉ' | 'EN_MAINTENANCE' | 'INDISPONIBLE',
    serialNumber: '',
    warranty: '',
    notes: ''
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      warranty: formData.warranty ? new Date(formData.warranty) : undefined,
      notes: formData.notes.trim() || undefined,
      associationId: 'assoc1', // TODO: Récupérer depuis le contexte
      addedDate: new Date()
    });
    ...
    setFormData({
      name: '',
      category: MaterialCategoryEnum.AUTRE,
      description: '',
      condition: 'BON',
      location: '',
      purchaseDate: '',
...
export default AddMaterialResourceModal;
